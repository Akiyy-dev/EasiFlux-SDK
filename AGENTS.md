...
EasiFlux-SDK is a pure Python library (no server/GUI) for the EasiCoin exchange REST API. Source lives in `src/easicoin_sdk`, tests in `tests/`, and a runnable demo in `examples/basic_usage.py`.
...
- The pip distribution name is `EasiFlux-SDK` but the importable package is `easicoin_sdk`.
